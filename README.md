...
- `mcp-server/00-deployment.yaml`: image tag ต้องตรงกับ `backend/03-deployment.yaml` เสมอ (bump คู่กันอัตโนมัติจาก CI ตั้งแต่ release `0.0.19` เป็นต้นไป ซึ่งเป็น release แรกที่มี `./mcp-server` binary)
